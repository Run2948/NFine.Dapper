    ...
    public class RoleAuthorizeApp
    {
        ...
        public List<ModuleButtonEntity> GetButtonList(string roleId)
        {
            ...
            else
            {
                ...
                var authorizedata = service.GetList(t => t.F_ObjectId == roleId && t.F_ItemType == 2).ToList();
                foreach (var item in authorizedata)
                {
        ...
        public bool ActionValidate(string roleId, string moduleId, string action)
        {
            ...
            if (cachedata == null)
            {
                var moduledata = moduleApp.GetList();
                var buttondata = moduleButtonApp.GetList();
                var authorizedata = service.GetList(t => t.F_ObjectId == roleId).ToList();
                foreach (var item in authorizedata)
                {
