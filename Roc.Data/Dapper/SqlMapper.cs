...
namespace Roc.Data
{
    ...
    public struct CommandDefinition
    {
        ...
        static void WriteLog(string title, IDbCommand cmd)
        {
            if (cmd != null)
            {
                ...
                sb.Append("========================================================");
                sb.AppendLine();
                sb.AppendFormat("操作时间: {0}\r\n", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                sb.AppendFormat("CommandType: {0}\r\n", cmd.CommandType.ToString());
                sb.AppendFormat("CommandText: {0}\r\n", cmd.CommandText);
                sb.AppendFormat("Transaction: {0}\r\n", cmd.Transaction == null ? false : true);
                if (cmd.Parameters != null && cmd.Parameters.Count > 0)
                {
                    StringBuilder parameters = new StringBuilder(100);
                    foreach (IDbDataParameter item in cmd.Parameters)
                    {
                        parameters.AppendFormat("[Name: '{0}',Type: '{1}',Value: '{2}',Direction: '{3}']\r\n", item.ParameterName, item.DbType.ToString(), item.Value, item.Direction.ToString());
                    }
                    sb.AppendFormat("Parameters: {0}", parameters.ToString());
                }
                sb.Append("========================================================");
                sb.AppendLine();
                WriteLog("DAPPER_SQL_LOG", title, sb.ToString());
            }
        }

        static void WriteLog(string fileName, string title, string content)
        {
            var log = LogFactory.GetLogger(fileName);
            log.Info(content);
        }
    }
...
#pragma warning restore 618
        ...
        internal static DbType LookupDbType(Type type, string name)
        {
            ...
            return LookupDbType(type, name, out handler);
        }
        #endregion
        /// <summary>
